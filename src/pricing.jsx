// PRICING — 3D hover cards, mobile-first stacked layout
const igPlans = [
  {name:'Spark',tier:'Getting Started',n:'01',emoji:'✦',tag:'Perfect for beginners',price:79,popular:false,tagline:'For creators who are trying but not getting reach',feats:['Your Reels start reaching more people','Initial visibility boost for your content','Better engagement signals on your posts','Your profile starts getting noticed'],best:'New creators stuck with low views',delivery:'5–7 days',stripe:'https://buy.stripe.com/7sY9AMb5Z69z3Odeuq67S00'},
  {name:'Ignite',tier:'Start Getting Seen',n:'02',emoji:'⚡',tag:'Break your reach ceiling',price:199,popular:true,tagline:'For creators stuck with low reach',feats:['Strong visibility push for multiple Reels','Your content reaches a larger audience','Higher engagement on your posts','Your profile starts building momentum','Direction to improve your content performance','Priority campaign handling'],best:'Creators who feel stuck and want real movement',delivery:'7–10 days',stripe:'https://buy.stripe.com/3cI00cb5Z2Xn3Od3PM67S01'},
  {name:'Momentum',tier:'Grow Steadily',n:'03',emoji:'📈',tag:'Build consistent growth',price:399,popular:false,tagline:'For creators who want consistent growth',feats:['Continuous visibility for your content','Stronger engagement over time','Better audience targeting','Your profile grows more consistently','Content improvement direction','Reach across multiple posts','Growth tracking summary'],best:'Creators who want regular, consistent growth',delivery:'10–14 days',stripe:'https://buy.stripe.com/dRm28keib2XngAZ86267S02'},
  {name:'Influence',tier:'Become a Creator',n:'04',emoji:'👑',tag:'Build authority & presence',price:799,popular:false,tagline:'For creators serious about building a strong presence',feats:['Long-term visibility support','Strong reach and engagement growth','Profile positioning improvement','Clear content direction','Personal growth guidance','Audience targeting strategy','Multi-post promotion campaign','Detailed results summary'],best:'Creators ready to take content seriously',delivery:'14–21 days',stripe:'https://buy.stripe.com/bJe6oAa1VbtT5Wlbie67S03'},
  {name:'Icon',tier:'Authority',n:'05',emoji:'💎',tag:'Dominate your niche',price:1199,popular:false,tagline:'For creators building a media brand',feats:['Full-scale promotion system','Content + profile overhaul support','Long-term audience building','Profile positioning strategy','Personal growth roadmap','Audience targeting — advanced','Multi-post + multi-platform campaign','Collab & brand outreach support','Detailed performance summary'],best:'Creators building a serious media brand',delivery:'21–30 days',stripe:'https://buy.stripe.com/cNicMY2zt69z0C14TQ67S04'},
];
const ytPlans = [
  {name:'Foundation',tier:'Foundation',n:'01',emoji:'▶',tag:'Start getting traction',price:399,popular:false,tagline:'For new YouTube creators ready for real visibility',feats:['Initial visibility for your videos','More reach to your content','Better watch-time signals','Your channel starts getting attention'],best:'New YouTube creators',delivery:'14 days',stripe:'https://buy.stripe.com/fZufZafmfeG54Shcmi67S05'},
  {name:'Accelerate',tier:'Start Growing',n:'02',emoji:'🚀',tag:'Break through low views',price:799,popular:true,tagline:'For creators stuck with low views',feats:['Strong visibility push for your videos','Better audience reach across videos','Improved engagement signals','Your channel starts growing consistently','Watch-time improvement','Priority campaign handling'],best:'Creators stuck on low views who need momentum',delivery:'21 days',stripe:'https://buy.stripe.com/14A3cofmf7dDdoN0DA67S06'},
  {name:'Scale',tier:'Scale Your Channel',n:'03',emoji:'💎',tag:'Dominate your niche',price:1299,popular:false,tagline:'For creators building a serious YouTube presence',feats:['Long-term growth support','Strong audience building','Better reach and retention signals','Clear growth direction','Multi-video promotion campaign','Channel positioning strategy','Detailed performance summary'],best:'Creators serious about YouTube as a career',delivery:'30 days',stripe:'https://buy.stripe.com/7sYaEQ8XRcxX4Shbie67S07'},
];
    ...
    <window.Section id="pricing" padded>
      <div className="wrap">
        <header style={{textAlign:'center', maxWidth:900, margin:'0 auto 44px'}}>
          ...
          <p className="reveal reveal-d2" style={{marginTop:12, fontSize:16, color:'var(--ink-2)', maxWidth:500, margin:'12px auto 0', fontFamily:'var(--serif)', fontStyle:'italic', fontWeight:300}}>
        ...
        <div className="price-mobile-scroll" style={{display:'none', overflowX:'auto', gap:12, paddingBottom:20, scrollSnapType:'x mandatory', WebkitOverflowScrolling:'touch', scrollbarWidth:'none', marginLeft:-20, marginRight:-20, paddingLeft:20, paddingRight:20}}>
          {plans.map((p, i) => <PriceCard key={`${platform}-mob-${i}`} plan={p} showCount={FEAT_SHOW[p.name]||p.feats.length} mobile/>)}
        </div>
        ...
        {/* Undecided CTA */}
        <div className="reveal" style={{marginTop:28, padding:'18px 22px', background:'rgba(22,101,52,.06)', border:'1px solid rgba(22,101,52,.15)', borderRadius:16, display:'flex', alignItems:'center', justifyContent:'space-between', gap:16, flexWrap:'wrap'}}>
  ...
  return (
    <div ref={ref} className="price-card-inner"
      ...
      style={{
        ...
        opacity: vis ? 1 : 0,
        transform: vis
          ? (hovered ? `perspective(700px) rotateY(${tilt.x}deg) rotateX(${tilt.y}deg) translateY(-4px) scale(1.02)` : 'perspective(700px) rotateY(0) rotateX(0)')
          : 'translateY(20px)',
        transition: vis ? 'opacity .5s, transform .35s cubic-bezier(.2,.7,.2,1), box-shadow .25s' : 'opacity .5s, transform .5s',
        boxShadow: hovered ? `${-tilt.x*.5}px ${tilt.y*.5}px 32px rgba(22,101,52,.15)` : isPop?'0 4px 24px rgba(22,101,52,.12)':'0 2px 12px rgba(15,31,15,.05)',
      ...
      <h3 style={{fontFamily:'var(--sans)', fontWeight:900, fontSize:'clamp(20px,2.2vw,28px)', letterSpacing:'-.03em', lineHeight:1, color:'var(--ink)', marginBottom:4}}>{p.name}</h3>
      <p style={{fontSize:11, color:'var(--ink-3)', marginBottom:10, lineHeight:1.4, fontStyle:'italic', fontFamily:'var(--serif)'}}>{p.tagline}</p>

      <div style={{display:'flex', alignItems:'baseline', gap:5, marginBottom:2}}>
        <span style={{fontFamily:'var(--sans)', fontWeight:900, fontSize:'clamp(24px,2.8vw,36px)', lineHeight:1, color:'var(--accent)', letterSpacing:'-.03em'}}>${p.price}</span>
        <span style={{fontSize:10, color:'var(--ink-3)', fontFamily:'var(--mono)'}}>one-time</span>
      </div>
      <div style={{fontSize:9, fontFamily:'var(--mono)', color:'var(--ink-3)', letterSpacing:'.06em', textTransform:'uppercase', marginBottom:10}}>Delivered in {p.delivery} · Launch pricing</div>

      <div style={{height:1, background:'var(--line)', marginBottom:9}}/>
      ...
      <ul style={{listStyle:'none', display:'flex', flexDirection:'column', gap:5, marginBottom:10, flex:1}}>
        {p.feats.slice(0, showCount).map((f, i) => (
          <li key={i} style={{display:'flex', gap:6, fontSize:12, lineHeight:1.4, color:'var(--ink-2)'}}>
      ...
      <div style={{padding:'7px 9px', borderRadius:8, marginBottom:9, background:'rgba(22,101,52,.06)', border:'1px solid rgba(22,101,52,.12)'}}>
