// WHO — 3D scroll-reveal cards, mobile-first
const audiences = [
  ...
  <window.Section id="who" padded>
    <div className="wrap">
      <div style={{marginBottom:40}}>
        ...
        <h2 className="wreveal" style={{...window.bigHeadStyle(), fontSize:'clamp(36px,6.5vw,100px)', marginTop:14}}>
...
const WhoCard = ({n, t, tag, d, cta, icon, delay}) => {
  ...
  return (
    <div ref={ref} className="who-card"
      ...
      style={{
        ...
        opacity: vis ? 1 : 0,
        transform: vis
          ? (hovered ? `perspective(700px) rotateY(${tilt.x}deg) rotateX(${tilt.y}deg) translateY(-6px) scale(1.02)` : 'perspective(700px) rotateY(0) rotateX(0) translateY(0)')
          : `translateY(36px) scale(.97)`,
        transition: vis
          ? `opacity .6s ease ${delay}s, transform .4s cubic-bezier(.2,.7,.2,1), box-shadow .3s`
          : `opacity .6s ease ${delay}s, transform .6s cubic-bezier(.2,.7,.2,1) ${delay}s`,
        boxShadow: hovered
          ? `${-tilt.x*.6}px ${tilt.y*.6}px 28px rgba(22,101,52,.15), 0 1px 0 rgba(255,255,255,.8) inset`
          : '0 2px 12px rgba(15,31,15,.06)',
      ...
      <div style={{display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:18}}>
        ...
        <span style={{
          ...
          background:'var(--accent-l)', border:'1px solid rgba(22,101,52,.2)',
          fontSize:10, fontFamily:'var(--mono)', color:'var(--accent)',
          textTransform:'uppercase', letterSpacing:'.08em',
        }}>{tag}</span>
      </div>
      <h3 style={{fontFamily:'var(--sans)', fontWeight:800, fontSize:'clamp(17px,1.9vw,22px)', letterSpacing:'-.02em', lineHeight:1.15, color:'var(--ink)', marginBottom:10}}>{t}</h3>
