// SHOWCASE — added swipe hint + CTA below mobile carousel
const NICHES = [
  ...
  return (
    <window.Section id="showcase" padded>
      <div className="wrap">
          ...
          <div className="showcase-niche-list" style={{display:'flex', flexDirection:'column', gap:6}}>
            {NICHES.map((n, i) => (
              <button key={n.id}
                ...
                <div style={{flex:1}}>
                  <div style={{fontFamily:'var(--mono)', fontWeight:700, fontSize:12, letterSpacing:'.1em', color:active===i?'var(--accent)':'var(--ink)', transition:'color .25s'}}>{n.niche}</div>
                  <div style={{fontSize:12, color:'var(--ink-3)', marginTop:2, lineHeight:1.4, maxHeight:active===i?60:0, overflow:'hidden', transition:'max-height .35s cubic-bezier(.2,.8,.2,1)'}}>{n.desc}</div>
                </div>
                <span style={{color:active===i?'var(--accent)':'var(--ink-4)', fontSize:16, transition:'transform .25s,color .25s', transform:active===i?'translateX(4px)':'none'}}>→</span>
              </button>
            ))}
          </div>

          {/* RIGHT — sticky niche card */}
              ...
              <div style={{position:'relative', marginTop:32}}>
                <div style={{display:'flex', gap:20, marginBottom:24}}>
                  {[['Real accounts','No bots'],['Fast results','24–72h'],['Both platforms','IG + YT']].map(([t,s])=>(
                    <div key={t}><div style={{fontSize:12,fontWeight:700,color:'rgba(255,255,255,.9)'}}>{t}</div><div style={{fontSize:11,color:'rgba(255,255,255,.45)',fontFamily:'var(--mono)'}}>{s}</div></div>
                  ))}
                </div>
                <button onClick={()=>window.openAuditModal&&window.openAuditModal()} style={{
                  ...
                  backdropFilter:'blur(8px)',transition:'background .2s',WebkitTapHighlightColor:'transparent',
                }}
        ...
        <div className="showcase-mobile-scroll" style={{display:'none', marginTop:0}}>
          ...
          <div style={{
            ...
            background:'rgba(22,101,52,.07)', border:'1px solid rgba(22,101,52,.15)',
            width:'fit-content', margin:'0 auto 14px',
          }}>
            <span style={{fontSize:13}}>👈</span>
            <span style={{fontFamily:'var(--mono)', fontSize:10, letterSpacing:'.1em', textTransform:'uppercase', color:'var(--accent)'}}>Swipe to explore niches</span>
            <span style={{fontSize:13}}>👉</span>
          </div>
          ...
          onScroll={e=>{
            ...
            {NICHES.map((n,i)=>(
              <div key={n.id} style={{
                ...
                <div style={{position:'relative'}}>
                  ...
                  <div style={{fontFamily:'var(--mono)',fontSize:9,letterSpacing:'.14em',textTransform:'uppercase',color:'rgba(255,255,255,.4)',marginBottom:8}}>Niche {String(i+1).padStart(2,'0')} of {NICHES.length}</div>
                  <h3 style={{fontFamily:'var(--sans)',fontWeight:900,fontSize:32,letterSpacing:'-.03em',lineHeight:.95,color:'#fff',textTransform:'uppercase'}}>{n.niche}</h3>
                ...
                <div style={{position:'relative', marginTop:20}}>
                  <div style={{display:'flex', gap:16, marginBottom:16, flexWrap:'wrap'}}>
                    {[['Real accounts','No bots'],['Fast results','24–72h'],['IG + YT','Both']].map(([t,s])=>(
                      <div key={t}><div style={{fontSize:11,fontWeight:700,color:'rgba(255,255,255,.9)'}}>{t}</div><div style={{fontSize:10,color:'rgba(255,255,255,.4)',fontFamily:'var(--mono)'}}>{s}</div></div>
                    ))}
                  </div>
                  <button onClick={()=>window.openAuditModal&&window.openAuditModal()} style={{
                    background:'rgba(255,255,255,.15)',color:'#fff',border:'1px solid rgba(255,255,255,.25)',
            ...
            {/* End spacer so last card isn't clipped */}
          ...
          <div style={{display:'flex', justifyContent:'center', gap:6, marginTop:14}}>
            {NICHES.map((_,i)=>(
              <div key={i} style={{width:active===i?24:6,height:6,borderRadius:999,background:active===i?'var(--accent)':'var(--line)',transition:'all .3s'}}/>
            ))}
          </div>

          {/* ADDED: CTA below carousel */}
          <div style={{
            marginTop:28, padding:'20px 22px',
            background:'rgba(22,101,52,.06)', border:'1px solid rgba(22,101,52,.15)',
            borderRadius:16, textAlign:'center',
          }}>
            <p style={{fontSize:14, color:'var(--ink-2)', marginBottom:14, fontFamily:'var(--serif)', fontStyle:'italic', fontWeight:300}}>
              Whatever your niche — we've helped creators like you get seen.
            </p>
            <button onClick={()=>window.openAuditModal&&window.openAuditModal()} style={{
            ...
            <p style={{marginTop:8, fontSize:11, color:'var(--ink-4)', fontFamily:'var(--mono)'}}>Free · No passwords · 48h turnaround</p>
          </div>
        </div>
      ...
      <style>{`
        ...
        @media(max-width:480px){.showcase-niche-btn{padding:12px 14px !important}}
      `}</style>
    </window.Section>
  );
};
